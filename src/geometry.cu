...
Cell::Cell(float volume, unsigned int id)
    : _volume(volume), _id(id), _material(nullptr), _neighbor_cells(nullptr),
      _num_neighbors(0) {}

void Cell::setMaterial(Material *material) { _material = material; }
  ...
  for (unsigned int i = 0; i < _num_neighbors; ++i) {
    if (_neighbor_cells[i]._id == this->_id)
      throw std::runtime_error(
          "Cell id = " + std::to_string(_neighbor_cells[i]._id) +
...
__device__ Cell *Cell::getRandomNeighborCell(float particle_energy) {
  ...
  float *escape_probability_to_some_cell =
      {}; // call some function to calculate cell escape prob
  ...
  float random_number =
      0; // will fix that later once we have a random number generator
...
const Material *Cell::getMaterial() const { return _material; }
